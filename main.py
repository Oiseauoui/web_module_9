...
# from models import AuthorItem, QuoteItem # Import the models from models.py

client = MongoClient('mongodb+srv://oiseua:Kivusd4ST6eqoJDp@cluster0.no20xgq.mongodb.net/test?retryWrites=true&w=majority')
db = client['home_08']
...
class DataPipline:
    ...
    def process_item(self, item, spider):
        ...
        if 'fullname' in adapter.keys():
            db['authors'].insert_one(dict(adapter))
            self.authors.append(dict(adapter))
        if 'quote' in adapter.keys():
            db['quotes'].insert_one(dict(adapter))
